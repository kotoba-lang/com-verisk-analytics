(ns verisk_analytics.main
  "Kotodama WASM entrypoint for the Verisk_analytics clean-room actor (L4) — Clojure port.
  ...
  facts (`verisk_analytics.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "verisk_analytics")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "veriskan_acc"
    :fields [:holderId :type :balance :currency :status] :required [:type :balance]
    :coerce {:balance :float}          :refs {}}
   {:entity "Loan"        :plural "loans"        :id-prefix "veriskan_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"verisk_analytics.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "verisk_analytics-compat" :tier tier :entities entities} 200])
...
(defn start! [] :verisk_analytics-compat/ready)
